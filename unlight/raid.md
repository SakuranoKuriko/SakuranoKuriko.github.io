...
移动阶段 / 全距离 / 无1↑ 无1↑ 无1↑ / 获得1张特殊事件卡[^图11]：移动阶段使用，本次移动阶段技能造成的伤害+20，本事件卡数值视为20。
...
需要自己同时有ATK↑和DEF↑两个buff，用十穿上buff的过程俗称热机。
...
第二个狂战在剑9造好后。热机完成前拖条[^注11]等待狂战，通常是会要配合到20分或50分准时上的，尽量通过拖条调整进入下一回合的时间。
...
**死兽事件卡有移1**，不要让死兽拿到移2↑，否则会无法控制距离，因此造剑9时盾卡用纯盾、事件卡盾、盾特，最多只能使用1张公牌盾移（用了就会加大翻车几率）。

移动阶段等5~10秒再确定，因为涡BOSS会拖条。<font class="heimu" color=white>拖条移2拉远就为了躲你的罗刹架势</font>
